...
public class RenderSystem extends SortedIteratingSystem {
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        if (renderQueue.get(CoreCMHolder.layer.get(entity).layerName) == null) {
            renderQueue.put(CoreCMHolder.layer.get(entity).layerName, new Array<>());
        }

        renderQueue.get(CoreCMHolder.layer.get(entity).layerName).add(entity);
    }
    ...
    private void drawRenderQueue(String layerName) {
        if (renderQueue.get(layerName) != null) {
            for (Entity entity : renderQueue.get(layerName)) {
                TextureComponent tex = CoreCMHolder.texture.get(entity);
                TransformComponent trans = CoreCMHolder.transform.get(entity);
